...
void draw()
{
  ...
  if(!done && !turn && !think && !queued)
    play();
    
  if(!piece[0][15].exists || !piece[1][15].exists)
    done = true;
  ...
  if(done)
  {
    ...
    if(piece[1][15].exists)
      rect(piece[0][15].x * size, piece[0][15].y * size, size, size);
    else if(piece[0][15].exists)
      rect(piece[1][15].x * size, piece[1][15].y * size, size, size);
  }
}
...
void play()
{
  ...
  for(int i=0; i<16; i++)
    for(int j=0; j<8; j++)
      for(int k=0; k<8; k++)
        if(piece[1][i].exists && piece[1][i].possible(j, k))
  {
    if(optimal == null)
      optimal = alphac(piece[1][i].x, piece[1][i].y, j, k, piece[1][i].x, piece[1][i].y, j, k, depth - 1, null);
  ...
  if(optimal != null)
    square[optimal.sx][optimal.sy].piece.move(optimal.tx,optimal.ty);
...
Move alphac(int sx, int sy, int tx, int ty, int osx, int osy, int otx, int oty, int d, Move min)
{
  ...
  if(d == 0)
    worst = new Move(osx, osy, otx, oty, evaluate());
  ...
  else
  {
    for(int i=0; i<16; i++)
      for(int j=0; j<8; j++)
        for(int k=0; k<8; k++)
          if(piece[0][i].exists && piece[0][i].possible(j,k))
    {
      if(worst == null)
        worst = betac(piece[0][i].x, piece[0][i].y, j, k, osx, osy, otx, oty, d - 1, null);
      ...
      if(min != null && worst.score < min.score)
      {
        ...
        if(sy == 1)
          square[sx][sy].piece.moved = false;
        if(destroyed != null)
          destroyed.revive();
...
Move betac(int sx, int sy, int tx, int ty, int osx, int osy, int otx, int oty, int d, Move max)
{
  ...
  if(d == 0)
    best = new Move(osx, osy, otx, oty, evaluate());
  ...
  else
  {
    ...
    {
      if(best == null)
        best = alphac(piece[1][i].x, piece[1][i].y, j, k, osx, osy, otx, oty, d - 1, null);
